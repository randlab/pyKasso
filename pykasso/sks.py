...
class SKS():
    ...
    def get_fractures_numbers(self):
        ...
        """
        user = self.geology.fractures_numbers
        model = []
    ...
    def compute_karst_network(self, verbose = False):
        ...
        # Step 2 - Compute conduits for each generation & store nodes and edges for network
        self._compute_iterations_karst_network()

        # Step 3 - Calculate the karst network statistics indicators with karstnet and save karst network
        ...
        # Step 4 - Store all the relevant data for this network in dictionaries:
                ...
                for z in range(self.grid.nz):
                    for y in range(self.grid.ny):
                        for x in range(self.grid.nx):
                            geology = self.geology.data['geology']['data'][0][y][x] ################################## !      !!!!!!!!!!!!!!!! TROISIEME DIMENSION, commencer la
