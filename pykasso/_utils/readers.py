...
class ProjectReader():
    ...
    def __init__(self, project_directory: str, *args, **kwargs) -> None:
        """TODO"""
        # Reads the project.yaml file
        project_directory = project_directory.lower().strip("/")
        project_file = "/outputs/project.yaml"
        self.project_file_location = project_directory + project_file
        self._update_project()
        # Reconstructs the grid
        self.grid = self._get_grid()
    
    def _update_project(self) -> None:
        with open(self.project_file_location, "r") as f:
    ...
    def _get_grid_origin(self) -> tuple:
        x0 = self.project['grid']['x0']
        y0 = self.project['grid']['y0']
        z0 = self.project['grid']['z0']
        return (x0, y0, z0)
    ...
    def _get_grid_spacing(self) -> tuple:
        dx = self.project['grid']['dx']
        dy = self.project['grid']['dy']
        dz = self.project['grid']['dz']
        return (dx, dy, dz)
    ...
    def _get_simulation_data(self, n: int) -> dict:
        simulation_directory = self.project['simulations'][n]
        simulation_data_path = simulation_directory + 'results.pickle'
        simulation_data = self._read_pickle(simulation_data_path)
        return simulation_data
    
    def get_sims(self, ni: int = 0, nf: int = None) -> list:
        """TODO"""
        n_sims = self.project['n_simulations']
        list_sims = list(range(n_sims))
        selected_sims = list_sims[ni:nf]
        return selected_sims
    
    def get_first_sims(self, n: int) -> list:
        """TODO"""
        first_sims = self.get_sims(0, n)
        return first_sims
    
    def get_last_sims(self, n: int) -> list:
        """TODO"""
        last_sims = self.get_sims(-n, None)
        return last_sims
